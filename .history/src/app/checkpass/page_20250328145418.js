        ...
        {pasajeros.map((item, i) => (
          <div key={i} className="bg-white rounded-lg shadow p-4 flex flex-col gap-2">
            <div className="flex items-center gap-2 font-semibold text-gray-700">
              <UserIcon size={20} /> {item.nombre || "Nombre no disponible"}
            </div>
            <p><strong>Email:</strong> {item.email}</p>
            <p><strong>Destino:</strong> {item.destino}</p>
            <p><strong>Fecha:</strong> {item.fechadesde}</p>
            <p><strong>Cantidad:</strong> {item.cantidad}</p>
            <p><strong>Estado:</strong> <span className={`font-bold ${item.estado === 'Aprobado' ? 'text-green-600' : 'text-red-500'}`}>{item.estado}</span></p>
          </div>
        ))}
      </div>
    </div>
